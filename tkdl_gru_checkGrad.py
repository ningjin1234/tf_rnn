...
def getGruRegressionOps(batchSize=5, maxNumSteps=10, nNeurons=4, initEmbeddings=None, bias_trainable=False, learningRate=0.1):
    ...
    rnnCell = TkdlGRUCell(nNeurons, activation=tf.tanh)
...
def trainRnn(docs, labels, nNeurons, embeddingFile, initWeightFile=None, trainedWeightFile=None):
    ...
    inputTokens, inputLens, targets, prediction, loss, initAll, learningStep, gradients, debugInfo = getGruRegressionOps(batchSize=batchSize, maxNumSteps=maxNumSteps, 
